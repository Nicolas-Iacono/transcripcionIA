...
type Props = {
  visible: boolean;
  onDismiss: () => void;
  garante: Partial<EditarGaranteForm> | null;
  onSubmit?: (payload: EditarGaranteForm) => void | Promise<void>;
};

const emptyForm = (): EditarGaranteForm => ({
  id: '',
  nombre: '',
  apellido: '',
  telefono: '',
...
export default function EditarGaranteModal({ visible, onDismiss, garante, onSubmit }: Props) {
  const insets = useSafeAreaInsets();
  const [formData, setFormData] = useState<EditarGaranteForm>(emptyForm());
  const [loading, setLoading] = useState(false);
  const [errors, setErrors] = useState<Partial<Record<keyof EditarGaranteForm, string>>>({});
  ...
  useEffect(() => {
    ...
    setFormData({
      ...
      dni: garante.dni ?? '',
      cuit: garante.cuit ?? '',
      direccionResidencial: garante.direccionResidencial ?? '',
    });
    setErrors({});
  }, [visible, garante]);
  ...
  const validateForm = () => {
    const next: Partial<Record<keyof EditarGaranteForm, string>> = {};
  ...
  const setField = (name: keyof EditarGaranteForm, value: string) => {

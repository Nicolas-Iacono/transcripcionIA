...
const EditarPropietarioModal = ({ open, onClose, propietario, onPropietarioUpdated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await PropietarioApi.actualizarPropietario(processed);
      showSuccess('Propietario actualizado exitosamente');
      
      // Notificar al componente padre que se actualizó el propietario
      if (onPropietarioUpdated) {
        onPropietarioUpdated();
      }
      
      onClose();
    } catch (error) {
      console.error('Error al actualizar propietario:', error);
      showError('Error al actualizar el propietario. Intenta nuevamente.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Dialog 
      ...
      sx={{
        '& .MuiDialog-container': {
          alignItems: 'flex-end',   // lleva el dialogo al fondo
        },
      }}
      PaperProps={{
        sx: {
          m: 0,
          width: '100%',            // mejor que 100vw dentro del dialog
          position: 'relative',     // no hace falta fixed/absolute si alineamos contenedor
...
export default EditarPropietarioModal;
